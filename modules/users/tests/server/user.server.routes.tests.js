  ...
  it('should be able to change profile picture if signed in', function (done) {
    agent.post('/api/auth/signin')
      ...
      .end(function (signinErr, signinRes) {
        ...
        agent.post('/api/users/picture')
          .set('Authorization', 'JWT ' + signinRes.body.token)
          .attach('newProfilePicture', './public/dist/img/users/client/img/profile/default.png')
          .send(credentials)
          .expect(200)
          .end(function (userInfoErr, userInfoRes) {
  ...
  it('should not be able to change profile picture if attach a picture with a different field name', function (done) {
    agent.post('/api/auth/signin')
      ...
      .end(function (signinErr, signinRes) {
        ...
        agent.post('/api/users/picture')
          .set('Authorization', 'JWT ' + signinRes.body.token)
          .attach('fieldThatDoesntWork', './public/dist/img/users/client/img/profile/default.png')
          .send(credentials)
          .expect(400)
          .end(function (userInfoErr, userInfoRes) {
            done(userInfoErr);
